...
class ResNet(nn.Module):
    def __init__(self, ResBlock, layer_list, num_classes, num_channels=3, num_feature_dim=1):
        ...
        self.numfeat_fc2 = nn.Linear(512*ResBlock.expansion, 512*ResBlock.expansion)
    ...
    def forward(self, x, time):
        ...
        #### passing time feature to fully connected layer and fusing with image fully connected layer
        
        time = time.reshape(-1, 1,) 
    
        time = self.numfeat_fc(time)

        # Adding Non-linearity to the time feature
        time = self.leaky_relu(time)
        
        time = self.numfeat_fc2(time)
        
        time = self.leaky_relu(time)

        time = self.numfeat_fc3(time)
